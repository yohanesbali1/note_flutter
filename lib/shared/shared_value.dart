...
class Helper {
  ...
  Future<void> export_excel_detail_transaction(detail, transcation) async {
    ...
    for (var i = 0; i < detail.length; i++) {
      ...
      sheet.getRangeByName('D${i + 7}').setText(convertToIdr(item.price, 2));
      sheet
          .getRangeByName('E${i + 7}')
          .setText(convertToIdr((item.amount * item.price), 2));
    ...
    sheet.getRangeByName('E${detail.length + 8}').text = convertToIdr(total, 2);
    sheet
